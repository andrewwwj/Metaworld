...
class SawyerStickPushEnvV2(SawyerXYZEnv):
    ...
    def evaluate_state(
        ...
        success = float(np.linalg.norm(container - self._target_pos) <= self.TARGET_RADIUS)
    ...
    def compute_reward(
        ...
        stick_in_place_margin = float(
            np.linalg.norm(self.stick_init_pos - target) - self.TARGET_RADIUS
        )
        ...
        container_in_place_margin = float(
            np.linalg.norm(self.obj_init_pos - target) - self.TARGET_RADIUS
        )
        ...
        if (
            ...
            if container_to_target <= self.TARGET_RADIUS:
                reward = 10.0
        return (
            reward,
            tcp_to_stick,
