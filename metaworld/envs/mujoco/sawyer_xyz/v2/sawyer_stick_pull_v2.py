...
class SawyerStickPullEnvV2(SawyerXYZEnv):
    ...
    def evaluate_state(
        ...
        handle_to_target = np.linalg.norm(handle - self._target_pos)
    ...
    def _stick_is_inserted(
        ...
        return (
            (np.abs(end_of_stick[0] - handle[0]) >= 0)
    ...
    def compute_reward(
        ...
        success = (handle_to_target <= self.TARGET_RADIUS) and self._stick_is_inserted(handle, end_of_stick)
        if success:
            reward = 10.0
